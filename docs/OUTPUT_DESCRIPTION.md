...
| **Lifecycle** | `lifecycle:transition` | String | Event state | **Always `"complete"` in ProcessLog v1.x** |
...
**Resource Behavior**:
- Performance varies (some faster/slower) — the resource `speed` multiplier
- Capacity varies (some handle more cases) — the resource `capacity` weight
- (Workload fatigue / load effects are **not** modeled in v1.x — see Known Limitations §6)
...
6. **Resource fatigue, random delays, and peak-time slowdowns are not applied.** The config schema accepts `resource_fatigue`, `anomalies.random_delays`, and `anomalies.peak_times` blocks (and the validator checks their shape), but the v1.x duration model is `base × resource-speed × resource-consistency` only — these three load/time variability mechanisms are reserved and not yet implemented. Generated durations reflect activity duration ranges and resource speed/consistency profiles, not workload or time-of-day effects.
