...
Tests for the command-line interface (processlog command).
...
class TestCLIGenerate:
    ...
    def test_generate_all_formats(self, tmp_path):
        ...
        # XES is an optional extra (processlog[xes]); "all" includes it only
        # when pm4py is installed, otherwise it is skipped with a warning.
        import importlib.util

        if importlib.util.find_spec("pm4py") is not None:
            assert (output_dir / "events.xes").exists()
        else:
            assert not (output_dir / "events.xes").exists()
