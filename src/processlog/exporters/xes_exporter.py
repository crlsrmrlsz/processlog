...
def export_xes(df: pd.DataFrame, output_path: str | Path) -> None:
    ...
    # pm4py is an optional dependency (heavy: scipy + graphviz + networkx).
    # Import it lazily so CSV / JSON / Parquet generation never pays for it,
    # and fail with an actionable message if XES is requested without it.
    try:
        import pm4py
    except ImportError as e:
        raise ExportError(
            "XES export requires the optional 'pm4py' dependency, which is not "
            "installed. Install it with:  pip install 'processlog[xes]'"
        ) from e
