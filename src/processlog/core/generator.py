...
def generate_log(
    ...
    # Per-case arrival (start) timestamps spread across [start_date, end_date].
    case_arrivals = _compute_case_arrivals(config, num_cases, start_date, tzinfo, seed)
    ...
    for case_num in range(1, num_cases + 1):
        ...
        try:
            case_events = _generate_case(case_id, config, case_arrivals[case_num - 1], rng, resource_tracker, case_attributes)
...
def _compute_case_arrivals(
    config: ConfigDict, num_cases: int, start_date: datetime,
    tzinfo: ZoneInfo, seed: int | None
) -> List[datetime]:
    """
    Compute a per-case arrival (start) timestamp for every case.

    Cases arrive as a homogeneous Poisson process across the window
    [start_date, end_date]: drawing ``num_cases`` i.i.d. uniforms and
    sorting them yields the arrival times of a Poisson process conditioned
    on the count, giving natural day-to-day clustering with exactly
    ``num_cases`` arrivals. Each arrival is then snapped onto the configured
    business calendar via ``_advance_to_business_hours`` (so weekend / holiday
    / after-hours arrivals roll to the next business morning).

    A dedicated RNG, independent of the main generation stream, keeps every
    case's emergent content reproducible and unchanged when arrival spreading
    is enabled — only the start dates move.

    Backward compatible: if ``end_date`` is absent or not after
    ``start_date``, every case starts at ``start_date`` (legacy behaviour).

    Args:
        config: Configuration dictionary (reads ``end_date`` + ``working_hours``)
        num_cases: Number of cases to schedule
        start_date: First possible arrival (tz-aware)
        tzinfo: Timezone for parsing ``end_date``
        seed: Resolved RNG seed; the arrival stream is derived from it

    Returns:
        List of ``num_cases`` tz-aware arrival datetimes, ascending.
    """
    end_date_str = config.get("end_date")
    if not end_date_str:
        return [start_date] * num_cases
    end_date = datetime.fromisoformat(end_date_str).replace(tzinfo=tzinfo)
    span_seconds = (end_date - start_date).total_seconds()
    if span_seconds <= 0:
        return [start_date] * num_cases

    arrival_rng = random.Random(f"arrivals-{seed}")
    offsets = sorted(arrival_rng.random() for _ in range(num_cases))
    return [
        _advance_to_business_hours(
            start_date + timedelta(seconds=offset * span_seconds), config
        )
...
def _create_event(
    ...
    emit_labels = bool(config.get("emit_labels"))
    ...
    if activity_type == "human":
        ...
        if resource_pool_name:
            ...
            if selected_resource:
                # Workload is always tracked by id so capacity/fatigue stay stable;
                # emit_labels only swaps the externally-visible string.
                resource_tracker["workload"][selected_resource["id"]] = (
                    resource_tracker["workload"].get(selected_resource["id"], 0) + 1
                )
                if emit_labels:
                    resource = selected_resource.get("name") or selected_resource["id"]
                else:
    ...
    # Resolve the activity string emitted to the event (concept:name on export).
    # Internal flow + next_steps + variant tracking remain id-keyed.
    if emit_labels:
        activity_label = activity_def.get("name") or activity_id
    else:
