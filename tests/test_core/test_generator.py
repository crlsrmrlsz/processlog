...
# Inline config with a real arrival window (start_date < end_date) and a
# business calendar — exercises case-arrival spreading.
SPREAD_CONFIG_YAML = """
process_name: "Arrival Spread Test"
num_cases: 200
seed: 42
start_date: "2024-01-01"
end_date: "2024-06-30"
...
class TestCaseArrivals:
    """Case start times spread across [start_date, end_date] on business days."""

    @staticmethod
    def _case_starts(df):
        """First (earliest) timestamp of each case = its arrival time."""
        return df.groupby("case_id")["timestamp"].min()

    def test_arrivals_spread_across_many_days(self):
        """Cases no longer all start on the same day."""
        config = parse_yaml(SPREAD_CONFIG_YAML)
        df = generate_log(config, seed=42, num_cases=200)
        starts = self._case_starts(df)
        distinct_days = {ts.date() for ts in starts}
        assert len(distinct_days) >= 30

    def test_arrivals_fall_on_business_days_only(self):
        """No case starts on a weekend when the calendar is enabled."""
        config = parse_yaml(SPREAD_CONFIG_YAML)
        df = generate_log(config, seed=42, num_cases=200)
        starts = self._case_starts(df)
        assert all(ts.weekday() < 5 for ts in starts)

    def test_arrivals_within_configured_window(self):
        """Arrivals sit inside [start_date, end_date] (+ a business-snap buffer)."""
        config = parse_yaml(SPREAD_CONFIG_YAML)
        df = generate_log(config, seed=42, num_cases=200)
        starts = self._case_starts(df)
        assert starts.min() >= pd.Timestamp("2024-01-01", tz="UTC")
        assert starts.max() <= pd.Timestamp("2024-07-07", tz="UTC")

    def test_arrival_count_matches_num_cases(self):
        config = parse_yaml(SPREAD_CONFIG_YAML)
        df = generate_log(config, seed=42, num_cases=200)
        assert df["case_id"].nunique() == 200

    def test_arrivals_reproducible_with_seed(self):
        config = parse_yaml(SPREAD_CONFIG_YAML)
        a = self._case_starts(generate_log(config, seed=42, num_cases=200))
        b = self._case_starts(generate_log(config, seed=42, num_cases=200))
        assert list(a) == list(b)

    def test_no_end_date_keeps_legacy_single_start_day(self, minimal_config_yaml):
        """Backward compatible: without end_date, all cases start the same day."""
        config = parse_yaml(minimal_config_yaml)
        df = generate_log(config, seed=42, num_cases=20)
        starts = self._case_starts(df)
        assert len({ts.date() for ts in starts}) == 1
