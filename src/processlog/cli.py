...
def cmd_generate(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        # Resolve requested formats. XES needs the optional pm4py extra: on a
        # lean install we skip it (with a warning) when "all" was requested, and
        # fail fast with an actionable message if it was asked for explicitly.
        # CSV / JSON / Parquet never need pm4py.
        requested_all = args.format == ['all']
        formats = ['csv', 'parquet', 'json', 'xes'] if requested_all else list(args.format)

        if 'xes' in formats and importlib.util.find_spec("pm4py") is None:
            if requested_all:
                print("⚠ Skipping XES — optional 'pm4py' not installed "
                      "(install with: pip install 'processlog[xes]')")
                formats = [f for f in formats if f != 'xes']
            else:
                print("✗ Error: XES export requires the optional 'pm4py' "
                      "dependency. Install with: pip install 'processlog[xes]'")
                return 1
