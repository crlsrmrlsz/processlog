...
**Generate**:
```bash
processlog generate -c configs/examples/simple_process.yaml -n 100
```
...
**Generate**:
```bash
processlog generate -c configs/examples/baseline_process.yaml -n 1000 -s 42
```
...
**Generate**:
```bash
processlog generate -c configs/examples/high_variance_process.yaml -n 500
```
...
### 4. Loan Origination (`loan-origination.yaml`)
**Purpose**: The canonical demo dataset for [mining-lib](https://github.com/crlsrmrlsz/mining-lib)

A realistic 13-activity mortgage loan-origination process — branching into
approval, decline, and withdrawal paths — with resource pools, a working
calendar, and case-level attributes (applicant segment, channel, loan amount
band, purpose, region).

**Reproduces the shipped mining-lib demo log byte-for-byte** with `seed=42`:
```bash
processlog generate -c configs/examples/loan-origination.yaml -n 2500 -f all
```

**Ideal for**:
- Regenerating the public mining-lib demo data exactly
- Exercising downstream process-mining tools on a realistic, multi-attribute log
...
```bash
processlog validate -c configs/examples/simple_process.yaml
```

### View Process Information
Get a summary without generating:
```bash
processlog info -c configs/examples/baseline_process.yaml
```

### Generate All Formats
Create CSV, Parquet, JSON, and XES at once:
```bash
processlog generate -c configs/examples/high_variance_process.yaml -n 200 -f all
```

### Reproducible Generation
Use a fixed seed for identical results:
```bash
processlog generate -c configs/examples/baseline_process.yaml -n 100 -s 42
```
...
- Usage reference (CLI, Python API, config, output): `../../docs/USAGE.md`
