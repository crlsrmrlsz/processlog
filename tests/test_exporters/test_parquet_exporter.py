...
class TestExportParquet:
    ...
    def test_export_parquet_preserves_data_types(self):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Check data types. pandas >= 3.0 reads parquet string columns back
            # as StringDtype rather than object; accept either — the point is
            # that the case id is a string type and the cost is float.
            from pandas.api.types import is_string_dtype

            assert is_string_dtype(df_read["case:concept:name"])
