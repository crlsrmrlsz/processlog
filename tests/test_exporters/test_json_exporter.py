...
class TestExportJson:
    ...
    def test_export_json_preserves_subsecond_precision_matching_csv(self):
        """REGRESSION (Phase 31 / Phase 33 audit): the JSON exporter must keep
        sub-second precision and stay byte-identical to the CSV exporter.

        The previous strftime format ("%Y-%m-%d %H:%M:%S%:z") dropped the
        microseconds that the generator produces (durations are float hours),
        flooring JSON timestamps to whole seconds while CSV kept full precision.
        """
        from datetime import timezone, timedelta

        from processlog.exporters.csv_exporter import export_csv

        # tz-aware datetimes WITH microseconds, built directly the way the
        # generator does (timedelta arithmetic on a tz-aware start), so this
        # exercises the exporter — not pandas' string parser. One whole-second
        # business-hours snap + one mid-activity value with microseconds.
        est = timezone(timedelta(hours=-5))
        df_internal = pd.DataFrame(
            {
                "case_id": ["case_001", "case_001"],
                "activity": ["submitted", "review"],
                "timestamp": [
                    datetime(2024, 3, 1, 9, 0, 0, tzinfo=est),
                    datetime(2024, 3, 1, 15, 36, 2, 533107, tzinfo=est),
                ],
                "resource": [None, "clerk_002"],
                "lifecycle": ["complete", "complete"],
            }
        )
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            export_json(df_internal, json_path)
            export_csv(df_internal, csv_path)
            ...
            # Microseconds must survive in JSON ...
            assert json_ts[1] == "2024-03-01 15:36:02.533107-05:00"
            # ... and the JSON timestamps must be byte-identical to CSV's.
            assert json_ts == csv_ts
